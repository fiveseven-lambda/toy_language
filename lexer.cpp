...
std::template unique_ptr<token::Token> &Lexer::peek(){
    if(!peeked){
        ...
        if(first == EOF){
            ...
            peeked = std::template make_unique<token::Identifier>(std::move(ret));
        }else if(first == '+'){
            peeked = std::template make_unique<token::Plus>();
        }else if(first == '-'){
            peeked = std::template make_unique<token::Hyphen>();
        }else if(first == '*'){
            peeked = std::template make_unique<token::Asterisk>();
        }else if(first == '/'){
            switch(input.peek()){
                case '*':
                    input.get();
                ...
                case '/':
                    ...
                    return peek();
                default:
                    peeked = std::template make_unique<token::Slash>();
            }
        }else if(std::isdigit(first)){ 
            std::string ret(1, std::string::traits_type::to_char_type(first));
            while(std::isdigit(input.peek())) ret.push_back(std::string::traits_type::to_char_type(input.get()));
            peeked = std::template make_unique<token::Integer>(std::move(ret));
        }
    }
    return peeked.value();
}

std::template unique_ptr<token::Token> Lexer::next(){
    peek();
